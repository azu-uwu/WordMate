...
### Phạm vi
- Đăng ký tài khoản mới với username, fullname, email, password.
...
| M2-T11 | Trang Đăng ký (Frontend) | Tạo `frontend/src/pages/auth/register.html` + `frontend/src/js/pages/register.js`. Form đăng ký: fullname, username, email, password, confirm password. Validate: fullname bắt buộc, username bắt buộc, email đúng định dạng, password >= 8, confirmPassword phải trùng password. Gọi POST /api/auth/register. Lưu JWT. Redirect sang chọn lộ trình nếu thành công | Authentication | ❌ | ✅ | ❌ | ❌ | P0 | M | M1-T8, M2-T2 | `frontend/src/pages/auth/register.html`, `frontend/src/js/pages/register.js` | Form register hiển thị, validate đúng, gọi API, lưu token. Lỗi validation → hiển thị lỗi |
